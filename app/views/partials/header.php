...
$isConfigModule   = (strpos($currentPath, '/nova1/modules/configuration/') === 0);
$isConfigMoodle   = (strpos($currentPath, '/nova1/modules/configuration/moodle/') === 0);

$isRegimeInstitutions = (strpos($currentPath, '/nova1/modules/school_regime/institutions/') === 0);
$isRegimeGroups       = (strpos($currentPath, '/nova1/modules/school_regime/groups/') === 0);
$isRegimePeriods      = (strpos($currentPath, '/nova1/modules/school_regime/periods/') === 0);
$isRegimeCourses      = (strpos($currentPath, '/nova1/modules/school_regime/courses/') === 0);
$isRegimeSubjects     = (strpos($currentPath, '/nova1/modules/school_regime/subjects/') === 0);
...
$canConfigMoodle = false;
$canSeeConfigMenu = false;

$canRegimeInstitutions = false;
$canRegimeGroups       = false;
$canRegimePeriods      = false;
$canRegimeCourses      = false;
$canRegimeSubjects     = false;
$canSeeRegimeMenu      = false;
...
if (isset($_SESSION['role_id'])) {
    ...
    if ($currentRoleId > 0) {
        ...
        $permissionsSql = "SELECT p.code
                           FROM role_permissions rp
                           INNER JOIN permissions p ON p.id = rp.permission_id
                           WHERE rp.role_id = ?
                             AND p.status = 'active'";

        if ($stmtPermissions = $connHeader->prepare($permissionsSql)) {
            $stmtPermissions->bind_param('i', $currentRoleId);
            $stmtPermissions->execute();
            $permissionsResult = $stmtPermissions->get_result();

            if ($permissionsResult) {
                while ($permissionRow = $permissionsResult->fetch_assoc()) {
                    $permissionCodes[] = $permissionRow['code'];
                }
            }
        ...
        foreach ($userPermissionCodes as $userPermissionCode) {
            if (in_array($userPermissionCode, $permissionCodes, true)) {
                $canSeeUsersMenu = true;
                break;
            }
        }

        $canRegimeInstitutions = in_array('school_regime.institutions.manage', $permissionCodes, true);
        $canRegimeGroups       = in_array('school_regime.groups.manage', $permissionCodes, true);
        $canRegimePeriods      = in_array('school_regime.periods.manage', $permissionCodes, true);
        $canRegimeCourses      = in_array('school_regime.courses.manage', $permissionCodes, true);
        $canRegimeSubjects     = in_array('school_regime.subjects.manage', $permissionCodes, true);
        $canConfigMoodle = in_array('configuration.moodle.manage', $permissionCodes, true);

        $canSeeRegimeMenu = (
            $canRegimeInstitutions ||
            $canRegimeGroups ||
            $canRegimePeriods ||
            $canRegimeCourses ||
            $canRegimeSubjects
...
$sidebarExpanded = (
    $isStudentsModule ||
    ($isRegimeModule && $canSeeRegimeMenu) ||
    ($isUsersModule && $canSeeUsersMenu) ||
    ($isConfigModule && $canSeeConfigMenu)
) ? 'menu-expanded' : '';
...
body.menu-expanded .sidebar{ width:var(--sidebar-expanded); }
body.menu-expanded .sidebar-top-fill{ width:var(--sidebar-expanded); }
body.menu-expanded .content{ margin-left:var(--sidebar-expanded); }
body.menu-expanded .context-fixed-wrap{ left:var(--sidebar-expanded); }
body.menu-expanded .topbar{ padding-left:calc(var(--sidebar-expanded) + 14px); }
...
.topbar-menu-btn i{ font-size:18px;color:#ffffff; }
...
.topbar-brand-name{ font-size:14px;font-weight:800;letter-spacing:.02em;color:#ffffff; }
.topbar-brand-version{ font-size:10px;font-weight:700;color:#93c5fd;margin-left:2px; }
.topbar-brand-sub{ font-size:10px;font-weight:700;color:#dbeafe;margin-top:2px;text-transform:uppercase; }
...
.topbar-logout:hover{ background:rgba(255,255,255,0.20); }
...
.topbar-user-icon i{ font-size:16px;color:#ffffff; }
...
.topbar-user-name{ font-size:12px;font-weight:700;color:#ffffff; }
.topbar-user-role{ font-size:11px;font-weight:700;color:#dbeafe;text-transform:uppercase; }
...
.sidebar::-webkit-scrollbar-thumb{ background:#22314d;border-radius:10px; }

.sidebar-section{ display:flex;flex-direction:column;gap:8px; }
.menu-entry{ display:flex;flex-direction:column;width:100%; }
...
.menu-link:hover,
.menu-toggle:hover{ background:var(--sidebar-hover);color:#ffffff; }

.menu-link.active,
.menu-toggle.active{ background:var(--sidebar-active);color:#ffffff; }
...
.menu-toggle.expanded .menu-caret{ transform:rotate(180deg); }
...
body.menu-expanded .menu-tooltip-target:hover::after{ display:none; }
...
.submenu-link span{ font-size:13px;font-weight:600; }
.submenu-link:hover{ background:#0c2345;color:#ffffff; }
...
@media (max-width: 900px){
    ...
    .topbar-center img{ height:34px; }

    :root{ --sidebar-expanded: 210px; }
}
</style>
</head>
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-section">
        ...
        <?php if ($canSeeRegimeMenu): ?>
            <div class="menu-entry">
                <button type="button"
                        id="regimeMenuButton"
                        class="menu-toggle menu-tooltip-target <?php echo $isRegimeModule ? 'active expanded' : ''; ?>"
                        data-title="Régimen Escolar"
                        onclick="toggleRegimeMenu()">
                    <i class="fa-solid fa-school menu-icon"></i>
                    <span class="menu-text">Régimen Escolar</span>
                    <i class="fa-solid fa-chevron-down menu-caret"></i>
                </button>

                <div id="regimeSubmenu" class="submenu <?php echo $isRegimeModule ? 'show' : ''; ?>">

                    <?php if ($canRegimeInstitutions): ?>
                        <a href="/nova1/modules/school_regime/institutions/index.php"
                           class="submenu-link <?php echo $isRegimeInstitutions ? 'active' : ''; ?>">
                            <i class="fa-solid fa-building-columns"></i>
                            <span>Institución</span>
                        </a>
                    <?php endif; ?>

                    <?php if ($canRegimeGroups): ?>
                        <a href="/nova1/modules/school_regime/groups/index.php"
                           class="submenu-link <?php echo $isRegimeGroups ? 'active' : ''; ?>">
                            <i class="fa-solid fa-layer-group"></i>
                            <span>Grupos</span>
                        </a>
                    <?php endif; ?>

                    <?php if ($canRegimePeriods): ?>
                        <a href="/nova1/modules/school_regime/periods/index.php"
                           class="submenu-link <?php echo $isRegimePeriods ? 'active' : ''; ?>">
                            <i class="fa-solid fa-calendar-days"></i>
                            <span>Períodos</span>
                        </a>
                    <?php endif; ?>

                    <?php if ($canRegimeCourses): ?>
                        <a href="/nova1/modules/school_regime/courses/index.php"
                           class="submenu-link <?php echo $isRegimeCourses ? 'active' : ''; ?>">
                            <i class="fa-solid fa-chalkboard-user"></i>
                            <span>Cursos</span>
                        </a>
                    <?php endif; ?>

                    <?php if ($canRegimeSubjects): ?>
                        <a href="/nova1/modules/school_regime/subjects/index.php"
                           class="submenu-link <?php echo $isRegimeSubjects ? 'active' : ''; ?>">
        ...
        <?php if ($canSeeConfigMenu): ?>
            <div class="menu-entry">
                <button type="button"
                        id="configMenuButton"
                        class="menu-toggle menu-tooltip-target <?php echo $isConfigModule ? 'active expanded' : ''; ?>"
                ...
                <div id="configSubmenu" class="submenu <?php echo $isConfigModule ? 'show' : ''; ?>">

                    <?php if ($canConfigMoodle): ?>
                        <a href="/nova1/modules/configuration/moodle/index.php"
                        class="submenu-link <?php echo $isConfigMoodle ? 'active' : ''; ?>">
...
function toggleSidebar() {
    ...
    const regimeSubmenu = document.getElementById('regimeSubmenu');
    const regimeTrigger = document.getElementById('regimeMenuButton');

    const usersSubmenu = document.getElementById('usersSubmenu');
    const usersTrigger = document.getElementById('usersMenuButton');

    if (isExpanded) {
        <?php if (!$isStudentsModule && !$isRegimeModule && !$isUsersModule): ?>
        if (studentsSubmenu && studentsTrigger) {
            studentsSubmenu.classList.remove('show');
            studentsTrigger.classList.remove('expanded');
        }
        if (regimeSubmenu && regimeTrigger) {
            regimeSubmenu.classList.remove('show');
            regimeTrigger.classList.remove('expanded');
        }
        ...
        <?php if ($isRegimeModule && $canSeeRegimeMenu): ?>
        if (regimeSubmenu && regimeTrigger) {
            regimeSubmenu.classList.add('show');
            regimeTrigger.classList.add('expanded');
        }
        <?php endif; ?>
...
function toggleRegimeMenu() {
    const submenu = document.getElementById('regimeSubmenu');
    const trigger = document.getElementById('regimeMenuButton');
...
function toggleConfigMenu() {
    ...
    const trigger = document.getElementById('configMenuButton');
...
function collapseSidebarMenu(event) {
    ...
    const configTrigger = document.getElementById('configMenuButton');
    ...
    if (configSubmenu && configTrigger) {
        configSubmenu.classList.remove('show');
        configTrigger.classList.remove('expanded');
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    <?php if ($isStudentsModule || ($isRegimeModule && $canSeeRegimeMenu) || ($isUsersModule && $canSeeUsersMenu)): ?>
    document.body.classList.add('menu-expanded');
    <?php else: ?>
    document.body.classList.remove('menu-expanded');
    <?php endif; ?>
    ...
    <?php if ($isConfigModule && $canSeeConfigMenu): ?>
    if (configSubmenu && configTrigger) {
        configSubmenu.classList.add('show');
        configTrigger.classList.add('expanded');
    }
    <?php else: ?>
    if (configSubmenu && configTrigger) {
        configSubmenu.classList.remove('show');
