...
if ($currentRoleId > 0) {
    ...
    if ($stmtPermission = $conn->prepare($permissionSql)) {
        ...
        $canEditUser = ($permissionResult && $permissionResult->num_rows > 0);
    ...
    <div style="max-width:900px;margin:0 auto;">
        ...
        ">
            ...
            <p style="margin:0;color:#6b7280;font-size:14px;">
                No tienes permisos para editar usuarios en NOVA 1.0.
            </p>
        </div>
    </div>
    <?php
...
$userSql = "SELECT id, role_id, username, first_name, last_name, email, phone, status
            FROM users
            WHERE id = ?
            LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($stmtCheckUser = $conn->prepare($checkUserSql)) {
            $stmtCheckUser->bind_param('si', $username, $userId);
            $stmtCheckUser->execute();
            $checkUserResult = $stmtCheckUser->get_result();

            if ($checkUserResult && $checkUserResult->num_rows > 0) {
                $errors[] = 'Ya existe otro usuario con ese nombre de acceso.';
            }
    ...
    if (empty($errors)) {
        if ($newPassword !== '') {
            ...
            if ($stmtUpdate = $conn->prepare($updateSql)) {
                ...
                $errors[] = 'No fue posible preparar la actualización del usuario.';
            }
        } else {
            $updateSql = "UPDATE users
                          SET role_id = ?,
...
<div style="max-width:1000px;margin:0 auto;">
    ...
    <div style="
        ...
        margin-bottom:16px;
    ">
        <h2 style="margin:0;font-size:18px;">Editar Usuario</h2>
        <p style="margin:4px 0 0 0;color:#6b7280;font-size:13px;">
            Modifica los datos del usuario en NOVA 1.0.
        </p>
    </div>
