...
if ($id <= 0 || $institutionId <= 0 || $code === '' || $name === '') {
    header('Location: /nova1/modules/school_regime/groups/index.php?error=required');
    exit;
}
...
$stmtCheck->bind_param('isi', $institutionId, $code, $id);
$stmtCheck->execute();
$resultCheck = $stmtCheck->get_result();

if ($resultCheck && $resultCheck->num_rows > 0) {
    $stmtCheck->close();
    header('Location: /nova1/modules/school_regime/groups/edit.php?id=' . $id . '&error=duplicate');
    exit;
}
...
try {
    ...
    header('Location: /nova1/modules/school_regime/groups/index.php?msg=updated');
    exit;

} catch (Exception $e) {
    $conn->rollback();

    header('Location: /nova1/modules/school_regime/groups/edit.php?id=' . $id . '&error=save');
    exit;
}