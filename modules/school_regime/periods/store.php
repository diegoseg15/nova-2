...
if ($studyGroupId <= 0 || $code === '' || $name === '' || $startDate === '' || $endDate === '') {
    header('Location: /nova1/modules/school_regime/periods/create.php?error=required');
    exit;
}

if ($endDate < $startDate) {
    header('Location: /nova1/modules/school_regime/periods/create.php?error=dates');
    exit;
}
...
if (!$group) {
    header('Location: /nova1/modules/school_regime/periods/create.php?error=group');
    exit;
}
...
$stmtCheck->bind_param('is', $studyGroupId, $code);
$stmtCheck->execute();
$resultCheck = $stmtCheck->get_result();

if ($resultCheck && $resultCheck->num_rows > 0) {
    $stmtCheck->close();
    header('Location: /nova1/modules/school_regime/periods/create.php?error=duplicate');
    exit;
}
...
try {
    if ($status === 'open') {
        $stmtClose = $conn->prepare("UPDATE periods SET status = 'closed' WHERE study_group_id = ? AND status = 'open'");
        $stmtClose->bind_param('i', $studyGroupId);
        $stmtClose->execute();
        $stmtClose->close();
    }

    $sqlInsert = "INSERT INTO periods
                    (institution_id, study_group_id, code, name, start_date, end_date, status, created_at, updated_at)
                  VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
    ...
    header('Location: /nova1/modules/school_regime/periods/index.php?msg=created');
    exit;

} catch (Exception $e) {
    $conn->rollback();

    header('Location: /nova1/modules/school_regime/periods/create.php?error=save');
    exit;
}