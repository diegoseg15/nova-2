...
$stmtCheck->bind_param('is', $courseId, $code);
$stmtCheck->execute();
$resultCheck = $stmtCheck->get_result();

if ($resultCheck && $resultCheck->num_rows > 0) {
    $stmtCheck->close();
    header('Location: /nova1/modules/school_regime/subjects/create.php?course_id=' . $courseId . '&error=duplicate');
    exit;
}
...
try {
    $sqlInsert = "INSERT INTO subjects
                    (course_id, code, name, description, cycle, status, created_at, updated_at)
                  VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())";
    ...
    header('Location: /nova1/modules/school_regime/subjects/index.php?course_id=' . $courseId . '&msg=created');
    exit;

} catch (Exception $e) {
    $conn->rollback();

    header('Location: /nova1/modules/school_regime/subjects/create.php?course_id=' . $courseId . '&error=save');
    exit;
}